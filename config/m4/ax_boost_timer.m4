# ===========================================================================
#     https://www.gnu.org/software/autoconf-archive/ax_boost_timer.html
# ===========================================================================
#
# SYNOPSIS
#
...
[
	...
	if test "x$want_boost" = "xyes"; then
        ...
        [AC_LANG_PUSH([C++])
			 ...
			 AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[@%:@include </usr/include/boost/timer/timer.hpp>]],
		...
		if test "x$ax_cv_boost_timer" = "xyes"; then
            ...
            if test "x$ax_boost_user_timer_lib" = "x"; then
                for libextension in `ls -r $BOOSTLIBDIR/libboost_timer* 2>/dev/null | sed 's,.*/lib,,' | sed 's,\..*,,'` ; do
                     ax_lib=${libextension}
                ...
                for libextension in `ls -r $BOOSTLIBDIR/boost_timer* 2>/dev/null | sed 's,.*/,,' | sed -e 's,\..*,,'` ; do
                     ax_lib=${libextension}
				    AC_CHECK_LIB($ax_lib, exit,
                                 [BOOST_TIMER_LIB="-l$ax_lib"; AC_SUBST(BOOST_TIMER_LIB) link_timer="yes"; break],
                                 [link_timer="no"])
